...
try{
	if (window.ActiveXObject !== undefined){  //兼容IE
	   ...
	   xmlDoc.load("../xml/img_address.xml"); 
		   ...
		   xmlhttp.send(null);  
		   xmlDoc = xmlhttp.responseXML?xmlhttp.responseXML:new DOMParser().parseFromString(xmlhttp.responseText,"text/xml"); 
		}
	}else{
		layui.use("layer", function () {
			var layer = layui.layer;
...
if(id != ''){
	$.ajax({ 
		  ...
		  success: function(data){
			  ...
			  $("#imgSelect").css("background-image",'url(../images/' + pictureData.icon_name + ')'); 
			  $("#iconId").attr("src","../images/"+pictureData.icon_name);
